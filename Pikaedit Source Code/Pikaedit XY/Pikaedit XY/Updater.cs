...
namespace Pikaedit_XY
{
    ...
    {
        private static readonly string url = "https://dl.dropboxusercontent.com/s/vhr2dyq2jch8gs0/versionXY.txt?dl=1";
        ...
        private static void downloadUpdate_DownloadFileCompleted(object sender, System.ComponentModel.AsyncCompletedEventArgs e)
        {
            ...
            //Update Plugins
            updatePlugins();
            System.Windows.Forms.MessageBox.Show("Update Downloaded! Pikaedit XY will restart", "Update Downloaded", System.Windows.Forms.MessageBoxButtons.OK);
            File.WriteAllText("info.bin", ver);
            File.WriteAllBytes("updater.exe", Properties.Resources.Updater);
            Process.Start("updater.exe", "\"Pikaedit XY\" info.bin");
            Environment.Exit(1);
        }
        ...
        private static void checkVersion_DownloadStringCompleted(object sender, DownloadStringCompletedEventArgs e)
        {
            ...
            log = log.TrimEnd('\n', '\r');
            s[0] = s[0].TrimEnd('\r').Replace("PikaEdit XY ", "");
            s[1] = s[1].TrimEnd('\r');
            file = "PikaEdit XY " + s[0] + ".exe";
            if (!s[0].Equals(v))
            {
                log = "Pikaedit XY has been updated to version " + s[0].TrimEnd('.', '0')/*.TrimStart('0','.')*/ + "\nVersion changes:\n" + log;
